...
class Indb(object):
    ...
    def get_hosts(self, table, db):
      ...
      url = self.api_url + '/query?q=SHOW+TAG+VALUES+FROM+%22' + table +'%22+WITH+KEY+%3D+%22host%22&db=' + db
      headers = {
      ...
      try:
          ...
          if resp.status_code == 401:
                raise IndbException(str(resp.status_code) + ':Authentication denied')
                return
          ...
          if resp.status_code == 404:
                raise IndbException(str(resp.status_code) +' :This request returns nothing.')
                return
    ...
    def get_data(self, table, db, host):
      ...
      url = self.api_url + '/query?q=select+*+from+' + table + '++where++host+%3D\''+  host +'\'&db=' + db
      headers = {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
    ...
    def get_data_24h(self, table, db, host):
      url = self.api_url + '/query?q=select+*+from+' + table + '+++where+time+%3E+now()+-+1d+and+host+%3D\''+  host +'\'&db=' + db
      headers = {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
    ...
    def get_data_24h_groupbytime(self, table, db, host):
      ...
      url = self.api_url + '/query?q=select+mean(value)+from+' + table + '+++where+time+%3E+now()+-+1d+and+host+%3D\''+  host +'\'+GROUP+BY+time(1h)&db=' + db
      headers = {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
    ...
    def ret_point_24h(self, table, db, host):
      ...
      for item in data:       
        timea = item[0]
        timeArray = datetime.datetime.strptime(timea, "%Y-%m-%dT%H:%M:%SZ") + datetime.timedelta(seconds=60*60*8)
